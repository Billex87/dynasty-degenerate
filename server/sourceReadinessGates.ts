...
export const SOURCE_READINESS_GATES: SourceReadinessGate[] = [
  ...
  },
  {
    ...
    evidence: {
      termsApproval: null,
      endpointPath: 'Probe covers official Fantasy Nerds docs candidates /v1/nfl/players, /teams, /draft-rankings, /adp, /dynasty, /draft-projections, /weekly-projections, /weekly-rankings, and /ros.',
      authModel: 'FANTASY_NERDS_API_KEY or FANTASYNERDS_API_KEY; pnpm run probe:fantasy-nerds uses only explicit keys. TEST fallback is development-only and not production evidence.',
      rowCount: 'June 5, 2026 local probe path exists but remains missing_config until an approved key is configured; require current-season non-TEST rows before production flags.',
      freshnessTimestamp: 'Probe captures source season and exposed freshness timestamp when present; blocked until current-season freshness is confirmed.',
      rateLimitResult: 'Probe detects HTTP errors and Fantasy Nerds 200 responses with rate-limit Error nodes; no approved package cadence result yet.',
      mappingCoverage: 'Player and team reference probes are included, but provider-to-Sleeper/team mapping coverage is unverified until protected payloads return current rows.',
      allowedAttributionLanguage: null,
    },
    nextAction: 'Run pnpm run probe:fantasy-nerds with approved package credentials; do not add or enable production features until current-season non-TEST rows, freshness, rate limits, mapping, and terms pass.',
  },
  {
    id: 'gridiron-data',
