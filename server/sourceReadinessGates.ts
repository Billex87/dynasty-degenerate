...
export const SOURCE_READINESS_GATES: SourceReadinessGate[] = [
  ...
  },
  {
    ...
    evidence: {
      ...
      rowCount: 'June 5, 2026 earlier metadata probe returned HTTP 200 with 597 rows, but the latest expanded/projection rerun hit 429 at rankings:DRAFT before projections ran; not approved for production model use.',
      freshnessTimestamp: 'Rows have been reachable in research probes, but stored snapshot fetchedAt/publishedAt/providerUpdatedAt evidence is not approved for production use.',
      rateLimitResult: 'June 5, 2026 latest full expanded/projection rerun returned 429 Too Many Requests at rankings:DRAFT and skipped remaining endpoints; normal cadence/rate limits are not validated.',
  ...
  },
  {
    ...
    evidence: {
      ...
      rowCount: 'Earlier June 5, 2026 Week 1 probe returned HTTP 200 with zero rows; latest expanded rerun skipped WW after a 429 at rankings:DRAFT.',
      freshnessTimestamp: 'Earlier June 5, 2026 Week 1 probe reported last_updated 1/01; required before use.',
      rateLimitResult: 'Requires closer-to-season paced check with non-zero rows and no 429.',
  ...
  },
  {
    ...
    evidence: {
      ...
      rowCount: 'Earlier June 5, 2026 metadata probe returned 403 Forbidden; latest expanded rerun skipped targets after a 429 at rankings:DRAFT.',
  ...
  },
  {
    ...
    evidence: {
      ...
      rowCount: 'Earlier June 5, 2026 metadata probe returned 403 Forbidden; latest expanded rerun skipped articles after a 429 at rankings:DRAFT.',
  ...
  },
  {
    ...
    evidence: {
      ...
      rowCount: 'Earlier June 5, 2026 metadata probe returned HTTP 200 with 25 rows; latest expanded rerun skipped news after a 429 at rankings:DRAFT. Production coverage still requires approval.',
      freshnessTimestamp: 'Required: publishedAt and snapshot updatedAt.',
      rateLimitResult: 'Required: production package rate-limit check; latest full expanded rerun hit 429 before news.',
  ...
  },
  {
    ...
    evidence: {
      termsApproval: null,
      endpointPath: 'Unknown until key/package access exists; candidate use cases are redraft projections, player news, or source-health benchmarking only.',
      authModel: 'Unknown key/package model; no integration or production env key is configured.',
      rowCount: 'No rows because no key/package access exists.',
      freshnessTimestamp: 'No freshness evidence until approved package rows are returned.',
      rateLimitResult: 'No rate-limit evidence until approved package access exists.',
      mappingCoverage: 'No player/team mapping evidence until endpoint payloads can be probed.',
      allowedAttributionLanguage: null,
    },
  ...
  {
    id: 'sleeper-hidden-account-transactions',
    source: 'Sleeper hidden account-level transaction import',
    status: 'blocked',
    normalReportLoad: 'blocked',
    publicClaimAllowed: false,
    evidence: {
      termsApproval: null,
      endpointPath: 'No official public endpoint for pending, cancelled, failed, rejected, skipped, or losing account-level waiver/trade rows was found in the June 5, 2026 Sleeper docs review.',
      authModel: 'Blocked: official Sleeper docs describe a read-only public API with no token auth; do not collect raw OAuth/session tokens or passwords.',
      rowCount: 'Public completed transactions can be read by league/week, but hidden pending/cancelled/skipped rows have no approved row source.',
      freshnessTimestamp: 'No hidden-account payload freshness evidence exists; completed public transactions expose created/status_updated only after they are visible.',
      rateLimitResult: 'Public docs warn to stay under documented API limits; no approved private/account-level cadence exists.',
      mappingCoverage: 'Public roster settings expose waiver_position and completed transactions expose roster/player IDs; losing/skipped/cancelled claim mapping is blocked.',
      allowedAttributionLanguage: null,
    },
    nextAction: 'Keep hidden import disabled until Sleeper provides an approved account authorization or partner API; use public completed transactions, manual labels, or explicit sanitized exports as fallback evidence.',
  },
  {
    ...
    evidence: {
      termsApproval: null,
      endpointPath: 'Unknown until an approved transaction feed is selected; current roster-room deltas use stored nflverse rosters, weekly rosters, depth charts, and trades.',
      authModel: 'Unknown; do not add a provider key or live transaction fetch until source approval exists.',
      rowCount: 'No official transaction-classification rows; current non-trade movement labels are inferred.',
      freshnessTimestamp: 'No official transaction freshness evidence; current roster-room snapshots expose generatedAt only.',
      rateLimitResult: 'No rate-limit evidence until an approved feed is selected.',
      mappingCoverage: 'Trades can use nflverse trade context, but signings, releases, waivers, reserve moves, and exact signing dates are not officially mapped.',
      allowedAttributionLanguage: null,
    },
