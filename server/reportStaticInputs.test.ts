...
vi.mock('./fantasyPros', () => ({
  fetchFantasyProsNews: mocks.fetchFantasyProsNews,
}));
...
const cachedPayload = {
  cacheKey: 'league-report-static-inputs-v1:12_sf_ppr_base:2026:2025',
...
describe('report static inputs cache loading', () => {
  beforeEach(() => {
    ...
    mocks.fetchFantasyProsNews.mockResolvedValue([{ playerName: 'A Player' }]);
    ...
    mocks.getUserLoadSnapshotOptions.mockReturnValue({ forceSnapshot: true });
  ...
  it('returns a fresh cached payload without calling snapshot loaders', async () => {
    ...
    expect(mocks.fetchFantasyProsNews).not.toHaveBeenCalled();
  ...
  it('loads and stores static inputs when the cache misses', async () => {
    ...
    expect(mocks.loadBlendedKTCValues).toHaveBeenCalledWith(input.leagueValueOptions, { forceSnapshot: true });
    expect(mocks.loadDraftSharksScheduleContext).toHaveBeenCalledWith({
      season: '2026',
      forceSnapshot: true,
    });
    expect(mocks.fetchFantasyProsNews).toHaveBeenCalledWith({ forceSnapshot: true });
...
describe('report static inputs', () => {
  it('keys static inputs by value profile and season window', () => {
    expect(getReportStaticInputsCacheKey({
      ...
      currentSeason: '2026',
      lastCompletedSeason: '2025',
    })).toBe('league-report-static-inputs-v1:12_sf_ppr_base:2026:2025');
  });
