...
- Historical player graphs and trade-date values use the generated `player-value-history-shards` store when available. That store is precomputed from the frozen archive and loaded by player shard, so user traffic does not call KTC, Flock, FantasyCalc, FantasyPros, or other weighted providers for historical values.
