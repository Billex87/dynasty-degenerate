...
- Server report-cache serving TTL defaults to 72 hours and can be tuned with `LEAGUE_REPORT_CACHE_TTL_HOURS`. The local file cache is pruned by the same TTL plus `LEAGUE_REPORT_FILE_CACHE_MAX_FILES` so stale fallback blobs do not accumulate.
- Historical player graphs and trade-date values use the generated `player-value-history-shards` store when available. That store is precomputed from the frozen archive and loaded by player shard, so user traffic does not call KTC, Flock, FantasyCalc, FantasyPros, or other weighted providers for historical values. Main report payloads keep slim timeline summaries plus selected-window points; the player modal hydrates full graph windows from static app assets or `VITE_VALUE_HISTORY_SHARDS_BASE_URL` only after the user opens the chart. Use `pnpm audit:value-history:shards` before deploy, and use `pnpm upload:value-history:shards` for S3/R2-compatible object storage when the shards are hosted outside the app.
- Browser report restore is fresh-cache-only and aligned to the 72-hour report-cache freshness window. The client keeps league-specific report copies in IndexedDB so switching back to a recently analyzed league can render immediately, but expired or old-version reports are not restored; current reports refresh in the background with a small header status instead of a blocking full-screen loader.
...
- `server/reportPlayerEnrichment.ts` caches player static enrichment maps by value profile, season window, and player-set signature. Normal cache hits skip rebuilding snapshot-backed value/news/history/schedule/prospect/similar-trade overlays; force refresh bypasses the cache when those static inputs need to be rebuilt immediately.
...
- Player enrichment cache hits must not provide live roster status, injury status, or display status. Those fields stay in the fresh base player-detail map.
