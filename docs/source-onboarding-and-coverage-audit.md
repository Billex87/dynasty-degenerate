...
| Dynasty Daddy source selector | public source-selector labels, player value-history field families, trade-volume/player-detail endpoints | research only | source disagreement diagnostics, ADP/value-over-cost benchmarking, competitor feature mapping | usage rights, upstream source attribution, rate limits, player-ID mapping, and privacy review for league-infused query params |
...
| Dynasty Daddy source selector | public SPA routes and `/api/v1/player/*` metadata endpoints | No auth observed for public player probes; terms unapproved | Research only; no report-load calls and no model use | source selector enum, market labels, player value-history fields, trade volume, rostership percentages, profile timestamp | player slug such as `lamarjacksonqb`; source-specific IDs not confirmed | `last_updated` in profile and per-row history `date` | Public endpoint access is not production permission; upstream field origins, attribution, cadence, rate limits, and Sleeper/GSIS mapping remain unapproved |
