...
| Weekly projections | Not approved for public provider-attributed production claims yet | Stored snapshots only when the source gate allows it | FantasyPros projections returned rows in an earlier June 5, 2026 metadata probe, but the latest expanded/projection rerun hit `429` at the first `rankings:DRAFT` request and skipped projection endpoints. Paid/public use still needs approved commercial terms, stored freshness metadata, normal cadence/rate-limit validation, mapping coverage, and legal/source approval. Sleeper weekly projections can be stored for readiness-gated internal mechanics but do not make public provider claims safe by themselves. |
