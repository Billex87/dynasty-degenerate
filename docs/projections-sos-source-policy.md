...
| FantasyPros matchup calendar / rank | Stored matchup snapshot when enabled | Cron/admin snapshot only | The public matchup calendar exposes player rank plus week-by-week matchup ratings for QB/RB/WR/TE/K/DST. Use rank as a relevance guard so low-ranked players do not surface just because one matchup is green. |
...
DraftSharks remains the approved-access SOS shell because it is partner-key gated, position/team based, and maps directly into bye, streamer, avoid-week, and schedule-tier fields. FantasyPros matchup pages are a separate matchup-calendar source, not season-long SOS: they refresh into stored admin/cron snapshots and feed short-window waiver and Rankings-tab matchup reads.
...
- FantasyPros matchup-calendar display should stay admin-only until attribution/display limits are finalized for public reports.
