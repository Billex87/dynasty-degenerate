...
Checklist status after this pass: `285` checked items and `156` unchecked items. The remaining unchecked items are primarily UI/Three.js, auth/billing/legal, schedule/SOS projection expansion, provider-access-dependent integrations, model/backtest work, deeper cache slimming, and future persistence features.
...
| Report load cache hits | Aligned browser report-cache lifetime with the server cache and stopped the report screen from calling `league.rankings` when the loaded report already includes rankings. | `client/src/pages/Home.tsx`, `todo.md` |
| Interactive value/ranking snapshot mode | Normal report generation, rankings generation, and multi-league rank lookup now load the latest stored source snapshots for value/ranking inputs instead of calling live KTC, FantasyCalc, DynastyProcess, Flock, DynastyNerds, FantasyPros dynasty/devy, or redraft ranking providers. | `server/ktcLoader.ts`, `server/rankingsBoard.ts`, `server/redraftRankings.ts`, `server/routers.ts`, `todo.md` |
| Interactive enrichment snapshot mode | FantasyPros news, ESPN depth charts, and DraftSharks/SOS now persist provider snapshots during dynamic-data refresh and read those snapshots during user-triggered report and player-detail loads. | `server/db.ts`, `server/providerDataSnapshots.ts`, `server/fantasyPros.ts`, `server/espnDepthCharts.ts`, `server/draftSharksSchedule.ts`, `server/dynamicDataJobs.ts`, `server/routers.ts`, `todo.md` |
| Sleeper season-stat snapshots | Aggregate Sleeper season stats now persist through dynamic-data refresh and normal report generation reads those snapshots for last-season ranks and historical availability instead of fetching static historical stat payloads. | `server/sleeperSeasonStats.ts`, `server/dynamicDataJobs.ts`, `server/routers.ts`, `server/sleeperSeasonStats.test.ts`, `todo.md` |
...
| Source audit template and matrix | Captured the current source coverage matrix, usage status, later feature potential, and open questions for Sleeper, FantasyPros, DraftSharks, KTC, Flock, FantasyCalc, Dynasty Nerds, DynastyProcess/nflverse, prospect sources, ESPN metadata, and internal jobs. | `docs/source-onboarding-and-coverage-audit.md` |
...
- Interactive value/ranking snapshot mode: implemented the first phase of source-snapshot-only report loads by keeping expensive value/ranking provider refreshes in jobs/manual refresh paths and using stored snapshots for user-triggered report/ranking reads.
- Interactive enrichment snapshot mode: extended the same pattern to non-Sleeper enrichments so FantasyPros news, ESPN depth charts, and DraftSharks/SOS are refreshed by jobs and read from stored provider snapshots during normal loads.
- Sleeper season-stat snapshots: kept live Sleeper calls for current league state, but moved static historical aggregate season stats behind persisted snapshots so report availability/rank calculations do not refetch old season payloads.
...
| Interactive report performance | Non-Sleeper value/ranking/news/depth-chart/SOS provider calls and static Sleeper season stats now read snapshots during normal loads. | Next performance work is payload splitting/slimming and tighter report-cache transfer, not provider call removal. |
