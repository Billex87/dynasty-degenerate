...
Checklist status after this pass: `283` checked items and `156` unchecked items. The remaining unchecked items are primarily UI/Three.js, auth/billing/legal, schedule/SOS projection expansion, provider-access-dependent integrations, model/backtest work, deeper cache slimming, and future persistence features.
...
| Interactive report performance | First phase is implemented for value/ranking providers. Remaining user-triggered non-Sleeper calls are news, depth-chart, schedule/SOS, and player-detail enrichment paths. | Move FantasyPros news, ESPN depth charts, DraftSharks/SOS schedule context, and any player-detail outside-Sleeper fetches to nightly snapshots or short-lived server caches. |
