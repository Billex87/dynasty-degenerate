...
| Current weekly riser/faller production recheck | The audit command is ready, but this shell has no `DATABASE_URL` and the local snapshot directory only has data through `2026-05-11`, so I could not verify a true May 14/15 production comparison here. | Run `pnpm audit:weekly-movement` in the production/snapshot environment after a May 14/15 or newer value snapshot exists. |
