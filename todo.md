...
- [ ] Add billing and entitlement tables for `billingCustomers`, `subscriptions`, `leaguePasses`, `featureEntitlements`, and `usageEvents`.
  ...
  - 2026-06-02 billing persistence helper scaffold: added server DB helpers for billing customer upserts, subscription upserts, subscription reads by user, billing customer lookup by user, idempotent usage-event writes, usage-event counting, and active feature-entitlement reads. Local tests cover no-database fail-safe behavior and required identifier validation. Remaining work: production migration execution, live DB verification, and route-level loading of persisted entitlement rows.
  - 2026-06-02 league-pass/entitlement persistence helper scaffold: added update-first helpers for `leaguePasses` and `featureEntitlements`, active user/league feature-entitlement reads, no-database fail-safe behavior, and required subject/league/user identifier validation. Remaining work: production migration execution, live DB verification, and route-level loading of persisted entitlement rows where paid features are enforced.
- [ ] Enforce paid access on the backend with a shared entitlement helper such as `canUseFeature(user, feature, leagueId)`; frontend paywalls should only be UX hints.
  ...
  - 2026-06-02 persisted-entitlement policy scaffold: `canUseFeature` now accepts active persisted user/league feature-entitlement rows, grants matching paid features only after `ENABLE_PAID_FEATURES=true`, respects league scope for League Pass entitlements, ignores expired/inactive rows, and still blocks admin diagnostics from persisted billing records. Remaining work: route-level DB loading for each paid feature surface, production DB verification, and real paid/free boundary QA.
