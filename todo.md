...
- [x] Before calling the product finished, run a production data/source freshness review: `pnpm audit:source-freshness`, `pnpm audit:league-report-cache`, admin source coverage, source-health logs, and provider telemetry; resolve recurring stale/missing/error sources or document accepted source limitations.
  ...
  - 2026-06-02 run: [docs/operations-readiness-pass-2026-06-02.md](docs/operations-readiness-pass-2026-06-02.md) refreshed the unified readiness snapshot after the AI/source-health guard work. Current state: 72 sources, 19 loaded, 47 stale, 6 missing, 0 error; coverage matrix 31 total, 14 loaded, 7 stale, 6 missing, 1 blocked, 3 research; source-health has 0 danger events and provider telemetry has 0 429s. Stale/missing providers remain accepted limitations gated by source-health confidence limits, not launch blockers.
- [ ] Before calling the product finished, run a Vercel production usage review after real traffic and at least one cron window: Fluid Active CPU, function invocations, transfer, provisioned memory, top function routes, skipped-cron behavior, and cached-report hit rate.
  ...
  - 2026-06-02 current-live smoke refresh: `npx --yes vercel@latest inspect dynastydegens.com` returned Ready deployment `dpl_2sMysEUVV6NAriRj6xhzFzRjuh5D`, created `Tue Jun 02 2026 10:57:43 GMT-0700`, which includes the AI source-trace guard but predates the latest league-rank cost commits through `0142a70`. Public production smoke passed `12`/`12` across desktop, tablet, and mobile for the four representative leagues. Deployment-scoped and production project log filters for error level, `500`, `/var/task`, `ENOENT`, failed file writes, league-report file-cache, `ApiProviderTelemetry`, missing-cookie, and normal report/headshot route traffic returned no rows in the checked `20m` window. Latest-code smoke remains pending until production catches up to `0142a70`.
  - 2026-06-02 latest-code production smoke refresh: `npx --yes vercel@latest inspect dynastydegens.com` returned Ready deployment `dpl_BmmbGZARfAV64tLqzKUgnpvSABCJ`, created `Tue Jun 02 2026 11:12:47 GMT-0700` after commit `30d21cd`, and aliased to `dynastydegens.com`. Public production smoke passed `12`/`12` across desktop, tablet, and mobile for the four representative leagues. Production log filters for error level, `500`, `/var/task`, `ENOENT`, failed file writes, league-report file-cache, `ApiProviderTelemetry`, missing-cookie, and normal report/headshot route traffic returned no rows in the checked `25m` window. The item remains open for dashboard-only CPU/usage metrics and future clean cron execution proof.
  - 2026-06-02 post-readiness-guard production smoke refresh: `npx --yes vercel@latest inspect dynastydegens.com` returned Ready deployment `dpl_3458jZfA1eiapx4oF4B4rdoWUoqq`, created `Tue Jun 02 2026 11:30:07 GMT-0700` after commit `2436cd4`, and aliased to `dynastydegens.com`. Public production smoke passed `12`/`12` across desktop, tablet, and mobile for the four representative leagues. Production log filters for error level, `500`, `/var/task`, `ENOENT`, failed file writes, league-report file-cache, `ApiProviderTelemetry`, missing-cookie, and normal report/headshot route traffic returned no rows in the checked `20m` window. The item remains open for dashboard-only CPU/usage metrics and future clean cron execution proof.
  - 2026-06-02 latest-push deployment gap: after commits through `e694249`, `dynastydegens.com` still pointed at Ready deployment `dpl_3458jZfA1eiapx4oF4B4rdoWUoqq`. A manual `npx --yes vercel@latest deploy --prod --yes --no-color` uploaded `1.8MB` but failed with Vercel's `api-deployments-free-per-day` limit (`more than 100`), so the newest source-agreement calibration commits are locally verified and pushed but not yet production-smoked.
  - 2026-06-02 latest AI/cost deploy gap: `npx --yes vercel@latest inspect dynastydegens.com --no-color` returned Ready deployment `dpl_6jngJL1r1W4vxsbNZQb3L1qnc47U`, created `Tue Jun 02 2026 11:47:06 GMT-0700`, which predates pushed commits `53e4f12`, `f029357`, and `13585ce`. `curl -I https://dynastydegens.com` returned `200` with `last-modified: Tue, 02 Jun 2026 18:58:35 GMT`, but latest-code production smoke/log verification remains pending until the Git deployment catches up or a clean production deploy can be run without including unrelated local Home/CSS worktree changes.
  - 2026-06-02 latest AI/cost production smoke refresh: `npx --yes vercel@latest inspect dynastydegens.com --no-color` returned Ready deployment `dpl_HPyT2eBKc8D3wyyfXZT7zubV84eF`, created `Tue Jun 02 2026 12:01:46 GMT-0700` after commit `9267c11`, and aliased to `dynastydegens.com`. Public production smoke passed `12`/`12` across desktop, tablet, and mobile for `Skids Get Beat`, `The Fantasy Degenerates`, `test league`, and `Gov Tech Grid Iron`. Deployment-scoped and production project log filters for error level, `500`, `/var/task`, `ENOENT`, failed file writes, league-report file-cache, `ApiProviderTelemetry`, missing-cookie, and normal report/headshot route traffic all returned no rows in the checked `20m` window, so visible error/local-write failures did not surface but CLI logs did not prove normal request rows. The item remains open for dashboard-only CPU/usage metrics and future clean cron execution proof.
...
- [ ] Before calling the product finished, run an AI-read correctness pass so visible recommendations cannot ask for impossible actions: already-starting players, unavailable players, locked games, injured/bye players, stale sources, redraft/dynasty mismatch, roster need already solved, duplicate/conflicting action reads, and missing evidence.
  ...
  - 2026-06-02 shared Do-this evidence-gap guard: `buildAIReadDecision` now refuses `Do this` when an evidence read still carries a confidence limit or missing-evidence gap, even if a future caller incorrectly sends `canAct: true`. Added direct decision-gate coverage for capped and unresolved-gap evidence payloads.
  - 2026-06-02 queue telemetry evidence-gap guard: AI prediction calibration telemetry now defensively downshifts malformed Action Queue rows from `do` to `watch` when missing evidence, missing source-health proof, or unhealthy source rows are present, and to `blocked` when blockers remain. Added direct coverage so persisted calibration events cannot learn from impossible queue actions if a future queue caller regresses.
  - 2026-06-02 server prediction event guard: `createAIPredictionEvent` now normalizes even caller-supplied `do` decisions back to `watch` or `blocked` when the evidence receipt still has missing proof, a confidence limit, hard blockers, or a decision-time baseline that the AI read failed to beat. Added server calibration coverage so stored outcome-memory events cannot learn from impossible action labels.
  - 2026-06-02 prediction summary effective-decision guard: reliability and counterfactual summaries now classify legacy malformed `do` rows by their effective safe decision, so older events with missing evidence, hard blockers, confidence limits, or failed baselines cannot inflate `do` hit-rate or missing-baseline action counts.
- [ ] Before calling the product finished, complete an operations/security pass: production env var names, `CRON_SECRET`, admin passphrase/session behavior, provider feature flags, API-key leak response steps, webhook/alert configuration, no secret logging, and rollback notes.
  ...
  - 2026-06-02 admin diagnostics account-wide rate guard: admin-only system diagnostics now key route-limit buckets by admin user instead of multiplying the allowance by caller IP, so one admin session cannot fan out expensive dashboard reads across multiple clients. Extended boundary coverage for the shared admin diagnostic limiter key.
  ...
  - 2026-06-02 league-rank target-user rate guard: `league.getUserLeagueRanks` now applies a shared Sleeper-user scoped limiter before cache reads and provider fanout, so repeated broad rank refreshes for one target user cannot multiply the expensive max-10 league enrichment path through ordinary client buckets.
...
- [ ] Add usage limits by tier, including reports per day, saved leagues, saved reports, alert count, export access, and source-trace visibility.
  ...
  - 2026-06-02 AI prediction account-wide rate guard: protected AI prediction write, history-read, and outcome-update routes now use the signed-in user key as the route-limit client bucket instead of multiplying the allowance by caller IP, so one account cannot fan out telemetry DB traffic across many clients.
...
- [ ] Run an AI Read System WOW + Correctness Pass before adding more AI surfaces. Treat AI reads as the highest-trust, biggest-wow product moment on the site: they must be visually premium, easy to understand, source-backed, state-aware, and correct enough that users trust the product after reading them.
  ...
  - [ ] Add automated regression coverage for AI-read correctness and duplication: text assertions for known real/fixture leagues, fixture tests for all action types, source-gating tests, mobile/desktop Playwright screenshots for Overview, Autopilot, Player Detail, Waiver, Trade, Rankings, and Draft surfaces, and a guard that flags multiple surfaces making the same action call.
    ...
    - 2026-06-02 source-detail zero-row wording guard: shared AI evidence and Autopilot source-health parsing now treat detail text like `0 row`, `0 rows`, `zero row`, and `zero rows` as missing source proof, so provider summaries cannot bypass unhealthy-source limits when row-count metadata is absent or malformed.
    - 2026-06-02 unavailable-source status guard: shared AI evidence now treats `unavailable` and `unverified` source-trace statuses as unhealthy source proof, matching Autopilot so direct action reads cannot bypass source-freshness limits when a provider is disabled or not verified.
    - 2026-06-02 waiver-mode calibration guard: shared AI evidence now preserves provided league `waiverMode` through context normalization, so FAAB/priority-specific calibration buckets can match waiver reads instead of silently falling back to broader outcome memory.
    - 2026-06-02 prediction source-agreement status guard: client AI prediction telemetry now classifies `unavailable` and `unverified` source traces as missing proof with zero source confidence, keeping outcome-calibration buckets aligned with the shared read-time source-health policy.
    - 2026-06-02 server source-agreement status guard: server calibration normalization now treats `unavailable` and `unverified` source-agreement signals as missing proof, so stored or replayed telemetry cannot regain directional source weight after the client read-time guard runs.
    - 2026-06-02 all-missing source-agreement guard: server calibration now classifies source-agreement reads with only missing/unavailable/unverified signals as `missing` instead of `unknown`, matching client telemetry and keeping missing-source buckets from being diluted.
    - 2026-06-02 disabled-source detail guard: shared AI evidence and client prediction telemetry now treat detail text like `provider disabled`, `source disabled`, `empty source`, and `source empty` as missing source proof even if a malformed trace reports `loaded`.
    - 2026-06-02 stale-source agreement guard: client and server prediction calibration now treat stale, error, limited, and zero-row source traces as missing source proof, so mixed loaded/unhealthy proof lands in split calibration buckets instead of regaining aligned or thin source confidence.
