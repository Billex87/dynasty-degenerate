...
- [ ] Once `Home.tsx` refactoring is no longer blocked, finish the homepage/report-entry cleanup: reduce the oversized page into stable feature components, keep route/data/cache behavior unchanged, and verify signed-out, recent-league, admin/view-as, cached-restore, and fresh-analysis paths.
  ...
  - 2026-06-01 fifteenth cleanup slice: extracted Sleeper username league lookup into `useHomeSleeperLeagueSearch` and report rankings hydration into `useHomeReportRankings`, preserving username validation, clown modal routing, Sleeper-session writes, admin-capable regular-view defaulting, league picker opening, rankings metadata fetch gating, and ranked report data passed to report/AI surfaces. Verified with `pnpm run check` and `PLAYWRIGHT_PORT=3128 pnpm exec playwright test tests/e2e/home.spec.ts tests/e2e/report-share-redraft.spec.ts tests/e2e/report-state-redraft.spec.ts --project=desktop-chrome` (`15` passed).
