...
test.describe('command center feature surfaces', () => {
  test('renders AI reads, blueprint generation, power rankings, roster recon, partners, and feature radar', async ({ page }) => {
    ...
    await expect(swapRead.locator('.manager-command-swap-out-name').getByText('Replace', { exact: true })).toBeVisible();
    ...
    await expect(swapRead.getByRole('button', { name: /Copy swap/i }).first()).toBeVisible();
    await expect(swapRead.getByRole('button', { name: /Open Sleeper/i }).first()).toBeVisible();
    await swapRead.getByRole('button', { name: /Save plan/i }).first().click();
    await expect(swapRead.getByRole('button', { name: /Plan saved/i }).first()).toBeVisible();
    await expect(page.getByText('Action History').first()).toBeVisible();
    const lineupPlans = await page.evaluate(() => JSON.parse(window.localStorage.getItem('dynasty-degenerates:action-plans:v1') || '[]') as Array<{ kind?: string }>);
    expect(lineupPlans.some((plan) => plan.kind === 'lineup')).toBeTruthy();
    await expect(page.locator('.manager-command-player-tile-swap-out')).toHaveCount(1);
    await expect(page.locator('.manager-command-player-tile-swap-in')).toHaveCount(1);
    await expect(page.locator('.manager-command-lock-status').first()).toBeVisible();
    ...
    const tradePartnerRead = page.locator('.trade-partner-card details.ai-read-panel-compact').first();
    await tradePartnerRead.evaluate((node) => node.setAttribute('open', ''));
    await tradePartnerRead.getByRole('button', { name: /Track trade read/i }).click();
    await expect(page.getByText('Tracked trade reads')).toBeVisible();
    const trackedTradePlans = await page.evaluate(() => JSON.parse(window.localStorage.getItem('dynasty-degenerates:action-plans:v1') || '[]') as Array<{ kind?: string; status?: string }>);
    expect(trackedTradePlans.some((plan) => plan.kind === 'trade' && plan.status === 'tracked')).toBeTruthy();
    ...
    const adminValueSection = await openReportSection(page, 'Admin Eyes Only: Value Assumptions');
    await expect(adminValueSection.getByText('Confidence Drilldown')).toBeVisible();
    await expect(adminValueSection.getByText('67% Building confidence').first()).toBeVisible();
    await expect(adminValueSection.getByText('League history: +12 to 54%').first()).toBeVisible();
    await expect(adminValueSection.getByText('Manager activity').first()).toBeVisible();
    await expect(adminValueSection.getByText('Depth chart roles')).toBeVisible();
    await expect(adminValueSection.getByText('6/8 players matched')).toBeVisible();
    await expect(adminValueSection.getByText('Team gaps')).toBeVisible();
    await expect(adminValueSection.getByText(/2 Sleeper role tags differed from the current team chart/i)).toBeVisible();
    await expect(adminValueSection.getByText(/Role enrichment took 428ms/i)).toBeVisible();
    await expect(adminValueSection.getByText(/Needs retry for: NYJ/i)).toBeVisible();
    await expect(adminValueSection.getByText('Sleeper history backfill')).toBeVisible();
    ...
    await expect(page.getByText(/Based on 2 WR bid samples/i).first()).toBeVisible();
    await expect(page.getByText(/Rival: 1 recent add\/drop move/i).first()).toBeVisible();
    await expect(page.getByText('Drop Last Bench Spot').first()).toBeVisible();
    await expect(page.getByRole('button', { name: /Copy plan/i }).first()).toBeVisible();
    await expect(page.getByRole('button', { name: /Open Sleeper/i }).first()).toBeVisible();
    await page.getByRole('button', { name: /Submit plan/i }).first().click();
    await expect(page.getByRole('button', { name: /Plan submitted/i }).first()).toBeVisible();
    await expect(page.getByText('Waiver Plan History').first()).toBeVisible();
    const submittedPlans = await page.evaluate(() => JSON.parse(window.localStorage.getItem('dynasty-degenerates:action-plans:v1') || '[]') as Array<{ kind?: string; playerId?: string }>);
    expect(submittedPlans.some((plan) => plan.kind === 'waiver' && plan.playerId === 'waiver1')).toBeTruthy();
    const bidHistory = await page.evaluate(() => JSON.parse(window.localStorage.getItem('dynasty-degenerates:waiver-bid-history:v1') || '[]') as Array<{ playerId?: string; bidMax?: number }>);
    expect(bidHistory.some((item) => item.playerId === 'waiver1' && Number(item.bidMax) > 0)).toBeTruthy();
